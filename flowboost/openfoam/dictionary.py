...
class DictionaryReader(Dictionary):
    ...
    def set(self, entry: str, new_value: Any) -> bool:
    ...
    def add(self, entry_path: str, value: Any) -> Optional['Entry']:
        ...
        print(
            f"Adding new entry with key={new_entry_key}, parent={current_parent.print_path()}")
        ...
        cmd = ["foamDictionary", self.path,
               "-entry", entry_path, "-add", foam_value]
        result = subprocess.run(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if result.stderr:
            logging.error(
                f"Error adding new entry '{entry_path}': {result.stderr}")
            return None
    ...
    def _discover_keywords(self):
        ...
        logging.debug(
            f"Discovering top-level keywords in Dictionary at {self.path}")
        ...
        if result.stderr:
            logging.error(
                f"Error discovering top-level keywords: {result.stderr}")
            return
...
class DictionaryLink:
    ...
    def reader(self, case_path: str | Path) -> Union[DictionaryReader, 'Entry']:
        """
...
class Entry:
    ...
    def __init__(self,
                 ...
                 parent: Optional['Entry'] = None) -> None:
        self.key: Optional[str] = key
        self._value: Optional[Any] = None
        self._raw_value: Optional[Any] = None

        # Dimensioning: e.g. [1 0 0 ...], [CAD]
    ...
    def index(self, i: int) -> Optional[Any]:
        ...
        if v is None:
            raise ValueError(
                "Entry that does not store a value cannot be indexed")
    ...
    def set(self, new_value: Any, override: bool = False) -> bool:
        """
        ...
        # Convert Pythonic value to FOAM value
        foam_value = FOAMType.to_FOAM(new_value)

        # For terminating entries or override
        cmd = ["foamDictionary", self.dictionary.path,
               "-entry", self.print_path(), "-set", foam_value]
        result = subprocess.run(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        ...
        self._raw_value = foam_value  # Store the new raw value as string
    ...
    def discover_subentries(self):
        ...
        if self.terminating is not False:
            ...
            cmd = ["foamDictionary", self.dictionary.path,
                   "-keywords", "-entry", self.print_path()]
            result = subprocess.run(
                cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if result.stderr:
            ...
            if result.stdout.strip():
                # Sub-entries exist
                self.keywords = [Entry(dictionary=self.dictionary, key=line, parent=self)
    ...
    def _discover_value(self):
        ...
        self._value = FOAMType.parse(data=self._raw_value)
...
def foamdictionary():
    """ Interface for running foamDictionary commands with error handling.
    """
